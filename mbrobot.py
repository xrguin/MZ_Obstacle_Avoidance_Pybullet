...
class MbRobot:
    ...
    def artificial_potential_field_uncertainty(self, sample_time, var_observation,
                                         attraction_factor, var_attraction, 
                                         repulsive_factor, var_repulsive):
        previous_head_angle = self.head_angle
        obstacle_distances = np.linalg.norm(self.obstacle - self.current_coord.reshape(-1, 1), axis=0)
        target_point = self.goal
        ...
        uncertain_obstacle = np.random.normal(self.obstacle[:, nearest_obstacle_index], np.sqrt(var_observation))

        obstacle_vector = self.current_coord - uncertain_obstacle
        ...
        random_attraction = np.random.normal(attraction_factor, np.sqrt(var_attraction))
        random_repulsion = np.random.normal(repulsive_factor, np.sqrt(var_repulsive))


        self.combined_force = random_attraction * self.attraction_force + random_repulsion * self.repulsion_force
        self.combined_force /= np.linalg.norm(self.combined_force)
